...
if page == "LOCATOR":
    ...
    st.title("LOCATOR")
    url = "10NIn_AJ9MiEnsnjqNFzoHaUxtLRaMKvy3QEAIPvcFpk"

    # Set up the credentials and client
    scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
    creds = ServiceAccountCredentials.from_json_keyfile_name("entry-form-430809-e197bdf6bd0c.json", scope)
    client = gspread.authorize(creds)

    # Open the Google Sheets document by URL and select the 'LOCATOR' sheet
    sheet = client.open_by_url(f"https://docs.google.com/spreadsheets/d/{url}")
    worksheet = sheet.worksheet("ALL FILES")

    # Fetch the data from the 'LOCATOR' sheet
    data = worksheet.get('A:I')
    ...
    search_product = st.selectbox("Product*", options=products, index=None)
    search_color = st.selectbox("Color*", options=color, index = None)
    ...
    st.dataframe(filtered_data[['POSITION NO.', 'INITIAL QTY']])
    ...
    styled_warehouse_df = warehouse_df.style.apply(lambda x: [highlight(v, x.index.get_loc(col)) for col, v in x.items()], axis=1)
